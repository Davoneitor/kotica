            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-3">
                <div class="flex gap-2 overflow-auto">
                    ...
                    <button class="px-4 py-2 rounded border text-sm whitespace-nowrap"
                            ...
                            @click="tab='ent'; ent.tipo=''; ent.desde=''; ent.hasta=''; ent.q=''; cargarEntradas()">
                        Entradas
                        <span x-show="tab==='ent' && entradas.length > 0"
                              class="ml-1 text-xs font-normal opacity-70"
                              x-text="'(' + entradas.length + ')'"></span>
                    </button>
...
<div x-show="tab==='ent'" class="mt-4 space-y-3">
    ...
    <div class="bg-white shadow-sm sm:rounded-lg p-4">
        ...
        {{-- Filtro por tipo (solo en tarjetas — en tabla los toggles están dentro de cada sección) --}}
        <div x-show="ent.vista==='tarjetas'" class="mt-3 flex flex-wrap items-center gap-2">
            <span class="text-xs text-gray-500 font-medium shrink-0">Tipo:</span>
            <button @click="ent.tipo=''; cargarEntradas()"
                    :class="ent.tipo==='' ? 'bg-gray-900 text-white' : 'bg-white hover:bg-gray-50 text-gray-700'"
                    class="px-3 py-1 rounded border text-sm transition-colors">Todos</button>
            <button @click="ent.tipo='oc'; cargarEntradas()"
                    :class="ent.tipo==='oc' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white hover:bg-emerald-50 text-emerald-700 border-emerald-300'"
                    class="px-3 py-1 rounded border text-sm transition-colors font-medium">🟢 Compra</button>
            <button @click="ent.tipo='manual'; cargarEntradas()"
                    :class="ent.tipo==='manual' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white hover:bg-blue-50 text-blue-700 border-blue-300'"
                    class="px-3 py-1 rounded border text-sm transition-colors font-medium">🔵 Manual</button>
            <button @click="ent.tipo='transferencia'; cargarEntradas()"
        ...
        <div x-show="!loading && entradas.length > 0" class="mt-3 flex flex-wrap gap-2 text-xs items-center">
            <span class="text-gray-500">Mostrando <strong x-text="entradas.length"></strong> entradas:</span>
            <span x-show="entradas.filter(e=>(!e.tipo||e.tipo==='oc')).length > 0"
                  class="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-semibold">
                🟢 <span x-text="entradas.filter(e=>(!e.tipo||e.tipo==='oc')).length"></span> compras
            </span>
            <span x-show="entradas.filter(e=>e.tipo==='manual').length > 0"
                  class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-semibold">
                🔵 <span x-text="entradas.filter(e=>e.tipo==='manual').length"></span> manuales
            </span>
            <span x-show="entradas.filter(e=>e.tipo==='transferencia').length > 0"
                  class="px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 font-semibold">
                🟠 <span x-text="entradas.filter(e=>e.tipo==='transferencia').length"></span> transferencias
            </span>
        </div>
        <div x-show="!loading && entradas.length === 0 && (ent.desde||ent.hasta||ent.tipo||ent.q)"
             class="mt-3 flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
            </svg>
            <span>No hay resultados con los filtros activos.</span>
            <button @click="ent.tipo=''; ent.desde=''; ent.hasta=''; ent.q=''; cargarEntradas()"
                    class="ml-auto font-semibold underline hover:no-underline">
                Limpiar filtros
            </button>
        </div>
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2">
            ...
            <button @click="ent.vista='tarjetas'"
                    ...
                    class="px-3 py-1 rounded border text-sm transition-colors">Tarjetas</button>
            <button @click="ent.vista='tabla'; if(ent.tipo){ ent.tipo=''; cargarEntradas(); }"
    ...
    <div x-show="ent.vista==='tarjetas'" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <template x-for="e in entradas" :key="e.id">
            <div class="bg-white shadow-sm rounded-lg border p-4">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            {{-- Tipo badge --}}
                            <span x-show="e.tipo === 'oc'"
                                  class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-emerald-100 text-emerald-700 border border-emerald-200">
                                🟢 Compra
                            </span>
                            <span x-show="e.tipo === 'manual'"
                            ...
                            <span x-show="e.tipo === 'transferencia'"
                                  class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-orange-100 text-orange-700 border border-orange-200">
                                🟠 Transferencia
                            </span>
                        </div>
                        <div class="text-sm text-gray-600 mt-0.5"
                             x-text="e.tipo === 'transferencia' ? (e.observaciones || 'Transferencia') : ('OC #' + e.id_pedido + ' · Det #' + e.pedido_det_id)">
                        </div>
                    </div>
                ...
                {{-- Badge de revertida --}}
                <div x-show="e.revertida" class="mt-2">
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-semibold bg-red-100 text-red-700 border border-red-200">
                        ⛔ Revertida
                        <span x-show="e.revertida_at" x-text="'· ' + formatFechaCorta(e.revertida_at)" class="font-normal"></span>
                    </span>
                    <div x-show="e.motivo_reversion" class="mt-1 text-xs text-red-600 italic" x-text="'Motivo: ' + e.motivo_reversion"></div>
                    <div x-show="e.revertida_por" class="text-xs text-red-500" x-text="'Por: ' + e.revertida_por"></div>
                </div>

                <div class="mt-2 text-xs text-gray-600" :class="e.revertida ? 'opacity-50 line-through' : ''">
                    <div x-show="e.tipo !== 'transferencia'">Fecha OC: <span class="font-medium" x-text="e.fecha_oc || '—'"></span></div>
                    <div>Recibido: <span class="font-medium" x-text="e.fecha_recibido"></span></div>
                    <div x-show="e.usuario">Usuario: <span class="font-medium" x-text="e.usuario"></span></div>
                    <div>
                        Foto:
                ...
                <div class="mt-3 flex gap-2">
                    <button class="flex-1 px-4 py-2 rounded bg-gray-900 text-white text-sm"
                            @click="verEntradaDetalles(e.id)">
                        Ver detalles
                    </button>
                    <template x-if="e.tipo === 'manual' && !e.revertida">
                        <button class="px-3 py-2 rounded border border-red-300 bg-red-50 text-red-700 hover:bg-red-100 text-sm font-medium transition-colors"
                                @click="abrirModalRevertir(e)">
                            Revertir
                        </button>
                    </template>
                </div>
    ...
    {{-- ========================= --}}
    <div x-show="ent.vista==='tabla'" class="space-y-4">

        {{-- 1. ÓRDENES DE COMPRA --}}
        <div id="sec-oc" x-show="ent.mostrar.oc" class="shadow-sm rounded-lg overflow-hidden" style="border:2px solid #059669">
...
{{-- ✅ Modal Revertir Entrada Manual --}}
<div x-show="revertirModal.show" x-cloak
     class="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4"
     @keydown.escape.window="revertirModal.show = false">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md" @click.stop>
        {{-- Header --}}
        <div class="flex items-center justify-between px-5 py-4 border-b bg-red-50 rounded-t-xl">
            <div>
                <div class="font-bold text-red-800 text-base">⛔ Revertir entrada manual</div>
                <div class="text-xs text-red-600 mt-0.5" x-show="revertirModal.entrada">
                    Entrada #<span x-text="revertirModal.entrada?.id"></span>
                    · <span x-text="revertirModal.entrada?.descripcion"></span>
                </div>
            </div>
            <button @click="revertirModal.show = false"
                    class="text-red-400 hover:text-red-600 p-1 rounded">✕</button>
        </div>

        {{-- Body --}}
        <div class="px-5 py-4 space-y-4">
            {{-- Resumen de lo que se revertirá --}}
            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">
                <div class="font-semibold text-amber-800 mb-1">Impacto en inventario</div>
                <div class="text-amber-700">
                    Se descontarán
                    <span class="font-bold" x-text="revertirModal.entrada?.cantidad_llego"></span>
                    <span x-text="revertirModal.entrada?.unidad"></span>
                    de <span class="font-semibold" x-text="revertirModal.entrada?.insumo || revertirModal.entrada?.descripcion"></span>
                </div>
                <div class="text-amber-600 text-xs mt-1">Esta acción no se puede deshacer automáticamente.</div>
            </div>

            {{-- Motivo (opcional) --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Motivo (opcional)</label>
                <textarea x-model="revertirModal.motivo"
                          rows="2"
                          placeholder="Ej: Error en captura, cantidad incorrecta..."
                          class="w-full border rounded-lg px-3 py-2 text-sm resize-none focus:ring-2 focus:ring-red-300 focus:outline-none"></textarea>
            </div>

            {{-- Error --}}
            <div x-show="revertirModal.error"
                 class="bg-red-50 border border-red-200 rounded p-3 text-sm text-red-700"
                 x-text="revertirModal.error"></div>
        </div>

        {{-- Footer --}}
        <div class="px-5 py-4 border-t flex gap-3">
            <button @click="revertirModal.show = false"
                    class="flex-1 px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm font-medium">
                Cancelar
            </button>
            <button @click="confirmarRevertir()"
                    :disabled="revertirModal.guardando"
                    class="flex-1 px-4 py-2 rounded-lg bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 text-sm font-medium flex items-center justify-center gap-2">
                <svg x-show="revertirModal.guardando" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                </svg>
                <span x-text="revertirModal.guardando ? 'Revirtiendo...' : 'Confirmar reverso'"></span>
            </button>
        </div>
    </div>
</div>
...
<div x-show="imgModal.show" x-cloak
    ...
    <img :src="imgModal.url" class="max-w-full max-h-[85vh] object-contain rounded bg-white">
        ...
        <script>
            ...
            function exploreUI() {
                return {
                    ent: { q:'', desde:'', hasta:'', vista:'tarjetas', tipo:'', mostrar:{ oc:true, manual:true, transferencia:true }, expandida:{ oc:true, manual:true, transferencia:true } },
                    entradas: [],
                    detallesEntradaId: null,
                    entradaDetalle: null,
                    imgModal: { show:false, url:'' },
                    revertirModal: { show:false, entrada:null, motivo:'', guardando:false, error:'' },
    ...
    try {
        ...
        this.loading = false;
                    ...
                    abrirModalRevertir(entrada) {
                        this.revertirModal = { show: true, entrada, motivo: '', guardando: false, error: '' };
                    },

                    async confirmarRevertir() {
                        const m = this.revertirModal;
                        m.guardando = true;
                        m.error     = '';
                        try {
                            const res = await fetchConCsrf(
                                `/explore/entradas/${m.entrada.id}/revertir`,
                                { motivo: m.motivo }
                            );
                            const data = await res.json();
                            if (!res.ok || !data.ok) {
                                m.error = data.message || 'Error al revertir.';
                                return;
                            }
                            m.show = false;
                            await this.cargarEntradas();
                        } catch (e) {
                            m.error = 'Error de conexión.';
                        } finally {
                            m.guardando = false;
                        }
                    },
